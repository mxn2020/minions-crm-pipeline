...
description: Agent skills for working with Minions Crm-pipeline MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-crm-pipeline data.
---

# Minions Crm-pipeline Agent Skills

Skills for agents operating on the `minions-crm-pipeline` toolbox.
...
```bash
# TypeScript
pnpm add @minions-crm-pipeline/sdk
...
The `crm-pipeline` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-crm-pipeline/sdk/schemas';
...
```python
from minions_crm_pipeline.schemas import custom_types
...
- This agent only writes to `minions-crm-pipeline` — it reads from other toolboxes but never writes to them
